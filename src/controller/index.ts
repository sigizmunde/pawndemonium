...
export class Controller {
    ...
    makeAMove(start: Figure | Position, stop: Position | Cell) {
        ...
        const destination = stopHasBoard ? stop : { board: figure.position!.board, cell: stop };
        if (!this.cellIsAvailable(destination)) {
            this._notify({ sender: 'controller', message: 'Impossible move, cell is closed' });
            return;
        }
        const enemy = this.findFigureInCell(destination);
        if (enemy) {
            this.killTheFigure(enemy);
            this._notify({ sender: 'controller', message: `${enemy.color} ${enemy.role} was killed by ${figure.color} ${figure.role}` });
        }
